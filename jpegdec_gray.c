...
static int create_vaapi_codec_ctx()
{
    ...
    attr.value.value.i = VA_FOURCC_Y800;
	format.fourcc = attr.value.value.i;
	...
	if (luma) {
		FILE *fp;
		printf("luma dump\n");
		status = vaCreateImage(vadpy, &format, width, height, &img);
		if(status != VA_STATUS_SUCCESS)
		{
			printf("vaCreateImage error : %s\n",vaErrorStr(status));
		}
		printf("-- planes = %d\n", img.num_planes);
		status = vaGetImage(vadpy,surfaces[0], x, y, width, height, img.image_id);
		if(status != VA_STATUS_SUCCESS)
		{
			printf("vaDeriveImage error : %s\n",vaErrorStr(status));
		}
